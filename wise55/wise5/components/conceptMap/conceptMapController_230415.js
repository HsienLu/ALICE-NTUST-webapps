    ...
    _createClass(ConceptMapController, [{
        ...
        value: function hideLinkTypeChooser() {
            if (this.newlyCreatedLink != null) {
                ...
                this.removeLink(this.newlyCreatedLink);
                this.newlyCreatedLink = null;
            }
            // hide the link type chooser
            this.displayLinkTypeChooser = false;
        ...
        value: function setupSVG() {
            var _this5 = this;
            var svg_element = document.getElementById(this.svgId)
            
            // get the svg element in the svg.js world
            this.draw = SVG(svg_element);
        ...
        value: function removeNode(node) {
            if (node) {
                // 刪除 node 的 outgoingLinks 與 incomingLinks
                const linksToRemove = node.getOutgoingLinks().concat(node.getIncomingLinks());
                this.removeAllLinks(linksToRemove);
        ...
        key: 'removeAllLinks',
        value: function removeAllLinks(links = null) {
            if (links == null) {
                // 刪除全部
                this.links.map(link => link.remove());
            ...
            if (event.target.parentElement != null) {
                ...
                if (groupId != null) {
                    ...
                    if (node != null) {
                        console.log(node)
                        // make the node highlighted
                        isMobile ? (this.clearActiveNode() && this.setActiveNode(node)) : '';
        ...
        value: function nodeMouseUp(event) {
            ...
            console.log('drawingLink', this.drawingLink)
            ...
            if (this.drawingLink && this.activeLink != null) {
                ...
                if (groupId != null) {
                    ...
                    if (node != null) {
                        console.log(node)
                        console.log('sourceNodeGroupId', sourceNodeGroupId)
                        console.log(sourceNodeGroupId)
        ...
        value: function linkMouseDown(event) {
            ...
            if (link != null) {
                isMobile ? (link.hideDeleteButton() && link.showDeleteButton()) : '';
        ...
        value: function showAutoFeedback() {
            ...
            // show the auto feedback in a modal dialog
            this.$mdDialog.show(this.$mdDialog.alert().parent(angular.element(document.querySelector('#feedbackDiv'))).clickOutsideToClose(true).title(this.$translate('FEEDBACK')).htmlContent(this.autoFeedbackString).ariaLabel(this.$translate('FEEDBACK')).ok(this.$translate('CLOSE')));
        }
